...
* **PROHIBITION ON HALLUCINATED PHYSICS:** You are expressly forbidden from inventing physical mechanics to explain behaviors (e.g., "compressing tires", "bouncing chassis"). If a maneuver fails, rely ONLY on the data logged (e.g., "one wheel received 0 power due to bad trim, causing a spin instead of a linear acceleration"). Do not invent "momentum swings" or "recoil" unless mathematically derived from IMU telemetry. If a simple acceleration fails to cause a flop, the only valid data-driven alternatives are accelerating in the opposite direction first, or verifying that both motors are actually receiving power before attempting the maneuver.
